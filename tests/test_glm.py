...
class TestGLM:
    ...
    def test_initialize_solver_mask_grouplasso(
        ...
        X, y, model, params, rate, mask = poissonGLM_model_instantiation_group_sparse
        model.set_params(
    ...
    def test_initialize_solver_all_invalid_X(
        ...
        with expectation:
            params = model.initialize_params(X, y)
            init_state = model.initialize_state(X, y, params)
            model.solver_update(params, init_state, X, y)
    ...
    #######################################
    # this is directly redudant with tests in test_overvation_models.py, since there is no way to call the deviance
    # from the GLM class directly.
    ...
    # TODO: need to check that population GLM checks are covered before removing this test
    @pytest.mark.parametrize(
...
class TestGLMShared:
    ...
    def test_parameter_initialization(self, X, y, request, model_instantiation):
    ...
    def test_fit_mask_grouplasso(self, request, model_instantiation):
        ...
        if model_instantiation == "poissonGLM_model_instantiation":
            X, y, model, params, rate, mask = request.getfixturevalue(
                model_instantiation + "_group_sparse"
            )
            model.set_params(
                regularizer_strength=1.0,
                regularizer=nmo.regularizer.GroupLasso(mask=mask),
    ...
    def test_fit_pytree_equivalence(self, request, model_instantiation):
        ...
        if model_instantiation == "bernoulliGLM_model_instantiation":
            # need to add this fixture
            return
    ...
    def test_score_type_r2(self, score_type, expectation, request, model_instantiation):
        """
    ...
    def test_loglikelihood_against_scipy_stats(
        ...
        if not np.allclose(mean_ll_jax, model_ll):
            raise ValueError(
                f"Log-likelihood of {model_instantiation} does not match"
                "that of jax.scipy!"
            )
    ...
    def test_update_n_samples(
        self, n_samples, expectation, batch_size, request, model_instantiation
    ):
        X, y, model, true_params, firing_rate = request.getfixturevalue(
            model_instantiation
        )
    ...
    @pytest.mark.parametrize("batch_size", [1, 10])
    def test_update_params_stored(self, batch_size, request, model_instantiation):
        X, y, model, true_params, firing_rate = request.getfixturevalue(
            model_instantiation
        )
        params = model.initialize_params(X, y)
    ...
    def test_update_nan_drop_at_jit_comp(
        self, batch_size, request, model_instantiation
    ):
    ...
    def test_simulate_pynapple(
        self, input_type, expected_out_type, request, model_instantiation
    ):
        """
    ...
    def test_simulate_feedforward_glm(self, request, model_instantiation):
        """Test that simulate goes through"""
        X, y, model, params, rate = request.getfixturevalue(model_instantiation)
