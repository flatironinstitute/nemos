...
class TestGLM:
    ...
    def test_fit_param_length(
        ...
        with expectation:
            model.set_kernel(X, y, init_params=init_params)
    ...
    def test_fit_param_values(
        ...
        if add_to == "X":
            ...
            y[idx] = add_entry
        with expectation:
            model.set_kernel(X, y, init_params=true_params)
    ...
    def test_fit_weights_dimensionality(
        ...
        with expectation:
            model.set_kernel(X, y, init_params=(init_w, true_params[1]))
    ...
    def test_fit_intercepts_dimensionality(
        ...
        init_w = jnp.zeros((n_features,))
        with expectation:
            model.set_kernel(X, y, init_params=(init_w, init_b))
    ...
    def test_fit_pytree_equivalence(
        ...
        # fit both models
        model.set_kernel(X, y, init_params=true_params)
        model_tree.set_kernel(X_tree, y, init_params=true_params_tree)
