...
| [`Newton`](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.PoissonRegressor.html) | scikit-learn | Newton method; each iteration builds the p × p Hessian (O(n · p²)) and solves it via Cholesky (O(p³)), where n = samples and p = features. Memory O(p²). Converges in very few iterations. CPU only. |
