...
eval_basis = additive_basis._compute_features(x_coord, y_coord)
...
axs[0].plot(x_coord, a_basis._compute_features(x_coord), "grey", alpha=.3)
axs[0].plot(x_coord, a_basis._compute_features(x_coord)[:, basis_a_element], "b")
axs[0].set_xlabel("x-coord")

axs[1].set_title(f"$b_{{{basis_b_element}}}(x)$", color="b")
axs[1].plot(y_coord, b_basis._compute_features(x_coord), "grey", alpha=.3)
axs[1].plot(y_coord, b_basis._compute_features(x_coord)[:, basis_b_element], "b")
...
eval_basis = prod_basis._compute_features(x_coord, y_coord)
...
for i, j in element_pairs:
    ...
    axs[cc, 0].plot(x_coord, a_basis._compute_features(x_coord), "grey", alpha=.3)
    axs[cc, 0].plot(x_coord, a_basis._compute_features(x_coord)[:, i], "b")
    axs[cc, 0].set_title(f"$a_{{{i}}}(x)$",color='b')

    # plot the element form b_basis
    axs[cc, 1].plot(y_coord, b_basis._compute_features(y_coord), "grey", alpha=.3)
    axs[cc, 1].plot(y_coord, b_basis._compute_features(y_coord)[:, j], "b")
...
eval_basis = prod_basis_3._compute_features(samples, samples, samples)
