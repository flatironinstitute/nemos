...
plt.plot(np.arange(spk_causal_utils.shape[0]), spk_causal_utils)
plt.ylabel('causal')

ax = plt.subplot(312)
ax.add_patch(rect_anticausal)
plt.vlines(np.arange(spk.shape[0]), 0, shift_spk[:, 0], color='k')
plt.plot(np.arange(spk_anticausal_utils.shape[0]), spk_anticausal_utils)
...
plt.vlines(np.arange(spk.shape[0]), 0, shift_spk[:, 0], color='k')
plt.plot(np.arange(spk_acausal_utils.shape[0]), spk_acausal_utils)
plt.ylabel('acausal')
plt.tight_layout()
