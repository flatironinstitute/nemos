...
- **`solver_state_`**: Indicates the solver's state. For specific solver states, refer to the solver implementations in `nemos.solvers`.
...
- [`initialize_optimization_and_state`](nemos.glm.GLM.initialize_optimization_and_state): Initialize the solver and its state. Takes initial parameters and returns the solver state.
