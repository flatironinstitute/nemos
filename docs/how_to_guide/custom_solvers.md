...
```{code-cell} ipython3
### NOTE: saving this here in case we decide that returning the closure is too complex.
# But I think for people writing their own solvers it should be fine.

# def _run_for_n_steps(self, params, n_steps: int, *args):
#    # params is a tuple of (weight_matrix, intercept)
#    # but scipy.optimize.minimize only accepts a 1D vector of parameters
#    # so flatten all parameters and concatenate them
#    orig_shapes = [x.shape for x in params]
#    sizes = [x.size for x in params]
#    split_indices = np.cumsum(sizes)[:-1]
#    _flat_params = np.concat([x.flatten() for x in params])

#    # create an objective function that translates from the flat parameters accepted by scipy
#    # back to their original form accepted by nemos.glm.GLM's objective function
#    def _flat_obj(flat_params, *args):
#        split_params = np.hsplit(flat_params, split_indices)
#        unstacked_params = tuple(
#            x.reshape(s)
#            for x, s in zip(split_params, orig_shapes)
#            # jnp.asarray(x.reshape(s))
#            # for x, s in zip(split_params, orig_shapes)
#        )
#        return self.fun(unstacked_params, *args)
...
#    split_final_params = np.hsplit(res.x, split_indices)
#    unstacked_final_params = tuple(
#        # jnp.asarray(x.reshape(s)) for x, s in zip(split_final_params, orig_shapes)
#        x.reshape(s)
#        for x, s in zip(split_final_params, orig_shapes)
#    )

#    return unstacked_final_params, res
```
