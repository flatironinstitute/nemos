...
class GLMRecurrent(GLM):
    def __init__(
        ...
        super().__init__(noise_model=noise_model, solver=solver, data_type=data_type)
    ...
    def simulate(
        ...
        (coupling_basis_matrix,) = self._convert_to_jnp_ndarray(
            coupling_basis_matrix, data_type=self.data_type
        )
...
#             &= -2 \left( \text{LL}\left(y\_{tn} | \hat{y}\_{tn}\right) -
#             \text{LL}\left(y\_{tn} | y\_{tn}\right)\right)
#         \end{aligned}
#         $$
#         where $ y $ is the observed data, $ \hat{y} $ is the predicted data, and $\text{LL}$ is the model
#         log-likelihood. Lower values of deviance indicate a better fit.
