...
class Basis(Base, abc.ABC, BasisTransformerMixin):
    ...
    def __init__(
        ...
        if (label is None) or (label == self.__class__.__name__):
            ...
            self._autogenerated_label = True

        else:
            self.label = str(label)
            self._autogenerated_label = False
    ...
    def set_params(self, **params: Any):
        # split basis params from other
        basis_params = {key: value for key, value in params.items() if isinstance(value, Basis)}
        # if not defaulted, label is inherited
        for key, value in basis_params.items():
            if value._autogenerated_label:
                # replace the label
    ...
    def label(self, label: str) -> None:
        ...
        self._label = None
        update_labels = self._sub_tree_update_labels([])
        if label in update_labels and not label.startswith(self.__class__.__name__):
            raise ValueError(f"Label {label} is already in use. When user-provided, label must be unique.")
        elif label in update_labels:
            # do not reverse the autogenerated
            self._label = self._generate_unique_key(update_labels, label)
        else:
            self._label = label
        self._autogenerated_label = False
        #self._super_tree_update_labels(old_label)

    # def _super_tree_update_labels(self, old_label: str) -> None:
    #     """
    #     Update parent labels.
    #
    #     If composite basis has a label of type: (basis1 + basis2) or (basis1 * basis2),
    #     and if the previous child label (``old_label``) is either basis1 or basis2,
    #     it will updated with the current child label.
    #
    #     Parameters
    #     ----------
    #     old_label:
    #         The previous child label. This should be iteratively replaced in the basis tree.
    #
    #     """
    #     parent = getattr(self, "_parent", None)
    #     # return if no parent or label of parent was user provided
    #     if parent is None or not parent._autogenerated_label:
    #         return
    #
    #     old_parent_label = deepcopy(parent._label)
    #     if ("(" + old_label) in old_parent_label:
    #         replace_string = "(" + old_label + " "
    #         new_string = "(" + self._label + " "
    #     elif (old_label + ")") in old_parent_label:
    #         replace_string = " " + old_label + ")"
    #         new_string = " " + self._label + ")"
    #     else:
    #         replace_string = None
    #     if isinstance(parent, CompositeBasisMixin) and replace_string:
    #         # strip parenthesis
    #         parent._label = parent._label.replace(replace_string, new_string)
    #         parent._super_tree_update_labels(old_parent_label)
...
class AdditiveBasis(CompositeBasisMixin, Basis):
    ...
    def __init__(self, basis1: Basis, basis2: Basis, label: Optional[str] = None) -> None:
