...
class Basis(Base, abc.ABC, BasisTransformerMixin):
    ...
    def set_params(self, **params: Any):
        ...
        basis_params = {
            key: value for key, value in params.items() if isinstance(value, Basis)
        }
        # if not defaulted, label is inherited
        for key, value in basis_params.items():
            if value._autogenerated_label:
                # replace the label
                self_params = self.__sklearn_get_params__(deep=True)
    ...
    def label(self, label: str) -> None:
        ...
        store_label = deepcopy(getattr(self, "_label", None))
        store_flag = getattr(self, "_autogenerated_label", True)
        self._label = None
        self._autogenerated_label = True

        # get root and list user-set labels
        update_labels = self._root()._list_subtree_labels()

        if label in update_labels and not label == self.__class__.__name__:
            # reverse the edits
            self._label = store_label
            self._autogenerated_label = store_flag
            raise ValueError(
                f"Label '{label}' is already in use. When user-provided, label must be unique."
            )
        else:
            self._label = label
        self._autogenerated_label = label == self.__class__.__name__
