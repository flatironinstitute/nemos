...
from typing import TYPE_CHECKING, Callable, Generator, Literal, Optional, Tuple, Union
...
class AtomicBasisMixin:
    ...
    @set_input_shape_state(states=("_input_shape_product", "_input_shape_", "_autogenerated_label", "_label"))
    ...
    def _list_subtree_labels(self, label_type: Literal["all", "user-set"] = "user-set"):
        """
        List all user-specified labels.
        """
        filter_label = label_type == "all" or not getattr(
            self, "_autogenerated_label", True
        )
        label_list = [self.label] if hasattr(self, "_label") and filter_label else []
        return label_list
...
class CompositeBasisMixin:
    ...
    def __init__(self, basis1: Basis, basis2: Basis, label: Optional[str] = None):
        ...
        # check labels
        shared_labels = set(basis1._root()._list_subtree_labels()) & set(
            basis2._root()._list_subtree_labels()
        )
    ...
    def _list_subtree_labels(self, label_type: Literal["all", "user-set"] = "user-set"):
        ...
        List all user-specified labels.
        """
        filter_label = hasattr(self, "_label") and (
            label_type == "all" or not getattr(self, "_autogenerated_label", True)
        )
        label_list = [self.label] if filter_label else []
        return (
            label_list
            + self.basis1._list_subtree_labels(label_type)
            + self.basis2._list_subtree_labels(label_type)
        )
    ...
    @set_input_shape_state(states=("_input_shape_product", "_autogenerated_label", "_label"))
    def __sklearn_clone__(self) -> Basis:
        """Clone the basis while preserving attributes related to input shapes.
