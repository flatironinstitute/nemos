...
_jit_delete = jax.jit(jnp.delete, static_argnames=["assume_unique_indices", "axis"])


@partial(jax.jit, static_argnums=(3,))
def _find_drop_column(feature_matrix, idx, rank, preprocessing_func=_add_constant):
    """Check if the column idx is linearly dependent from the other columns."""
    rank_after_drop_column = jnp.linalg.matrix_rank(
        preprocessing_func(
            _jit_delete(feature_matrix, idx, axis=1, assume_unique_indices=True)
        )
    )
...
def _search_and_drop_columns(state, find_drop_columns):
    ...
    feature_matrix = _jit_delete(
        feature_matrix, idx, axis=1, assume_unique_indices=True
    )
    return jnp.any(drop_cols), feature_matrix, rank, idx


def _apply_identifiability_constraints(
    feature_matrix: NDArray, preprocessing_func: Callable = _add_constant
):
    """
...
def apply_identifiability_constraints(
    ...
    kept_columns = np.delete(
        np.arange(feature_matrix.shape[1]), discarded_columns, axis=0
    )
...
def apply_identifiability_constraints_by_basis_component(
    ...
    shifts = list(
        np.cumsum(
            [0] + [sub_x.shape[2] for sub_x in jax.tree_util.tree_leaves(splits_x)[:-1]]
        )
    )
